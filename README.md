...
EDA Report (Markdown)
ML Recommendation Report (Markdown)
Cleaning Log (JSON)
Starter ML training script (`.py`)
...
Sign up for free at console.groq.com. The free tier is sufficient for all features.
4. Run the app
```bash
streamlit run app.py
```
Then open http://localhost:8501 and paste your Groq API key in the sidebar.
...
Instantly profile every column — types, missing values, cardinality, outliers
Score dataset health from 0–100 with a breakdown by category
Propose a cleaning plan with risk labels — you approve or reject each step individually
Execute cleaning deterministically via Pandas and log every action
Generate EDA charts with one-line AI insights per chart
Detect your target variable and assess ML readiness
Rank features by mutual information + correlation
Recommend models with pros, cons, and expected performance
Write a Senior Data Scientist review with actionable notes
